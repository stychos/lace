...
void db_register_driver(DbDriver *driver) {
    if (!driver || g_num_drivers >= MAX_DRIVERS) return;
...
DbConnection *db_connect(const char *connstr, char **err) {
    ...
    if (!driver) {
        if (err) *err = str_printf("Unknown driver: %s", cs->driver);
        connstr_free(cs);
        return NULL;
    }
...
bool db_ping(DbConnection *conn) {
    if (!conn || !conn->driver || !conn->driver->ping) return false;
