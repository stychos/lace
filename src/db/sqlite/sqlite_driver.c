...
static TableSchema *sqlite_get_table_schema(DbConnection *conn, const char *table, char **err);
...
static DbConnection *sqlite_connect(const char *connstr, char **err) {
    ...
    if (!str_eq(cs->driver, "sqlite")) {
        connstr_free(cs);
        if (err) *err = str_dup("Not a SQLite connection string");
        return NULL;
    }

    if (!cs->database || !cs->database[0]) {
        connstr_free(cs);
        if (err) *err = str_dup("No database path specified");
        return NULL;
    }
    ...
    if (rc != SQLITE_OK) {
        if (err) *err = str_printf("Failed to open database: %s",
...
static bool sqlite_ping(DbConnection *conn) {
    if (!conn || !conn->driver_data) return false;
    SqliteData *data = conn->driver_data;
    return data->db != NULL;
}

static ConnStatus sqlite_status(DbConnection *conn) {
    if (!conn) return CONN_STATUS_DISCONNECTED;
    return conn->status;
}
...
static char **sqlite_list_tables(DbConnection *conn, size_t *count, char **err) {
    ...
    *count = 0;
    ...
    if (rc != SQLITE_OK) {
        if (err) *err = str_printf("Query failed: %s", sqlite3_errmsg(data->db));
        return NULL;
    }
...
static TableSchema *sqlite_get_table_schema(DbConnection *conn, const char *table, char **err) {
    ...
    if (rc != SQLITE_OK) {
        if (err) *err = str_printf("Failed to get table info: %s", sqlite3_errmsg(data->db));
        db_schema_free(schema);
        return NULL;
    }
    ...
    if (num_cols == 0) {
        sqlite3_finalize(stmt);
        if (err) *err = str_dup("Table not found or has no columns");
        db_schema_free(schema);
        return NULL;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW && i < num_cols) {
        ...
        if (dflt) col->default_val = str_dup(dflt);

        /* Determine type from type name */
        const char *type_name = col->type_name;
        if (type_name) {
            if (strstr(type_name, "INT")) col->type = DB_TYPE_INT;
            else if (strstr(type_name, "REAL") || strstr(type_name, "FLOAT") ||
                     strstr(type_name, "DOUBLE")) col->type = DB_TYPE_FLOAT;
            else if (strstr(type_name, "BLOB")) col->type = DB_TYPE_BLOB;
            else if (strstr(type_name, "BOOL")) col->type = DB_TYPE_BOOL;
            else col->type = DB_TYPE_TEXT;
        }
                        ...
                        if (ncols > 0) {
                            ...
                            if (index->columns) {
                                ...
                                while (sqlite3_step(idx_stmt) == SQLITE_ROW && c < ncols) {
                                    const char *col_name = (const char *)sqlite3_column_text(idx_stmt, 2);
    ...
    if (num_cols > 0) {
        ...
        if (rs->columns) {
            ...
            for (int i = 0; i < num_cols; i++) {
                ...
                if (type) rs->columns[i].type_name = str_dup(type);
            }
        }
    }
...
static int64_t sqlite_exec(DbConnection *conn, const char *sql, char **err) {
    ...
    if (rc != SQLITE_OK) {
        if (err) *err = str_dup(errmsg ? errmsg : sqlite3_errmsg(data->db));
        sqlite3_free(errmsg);
        return -1;
    }
...
static bool sqlite_update_cell(DbConnection *conn, const char *table,
    ...
    if (rc != SQLITE_OK) {
        if (err) *err = str_printf("Failed to prepare statement: %s", sqlite3_errmsg(data->db));
        return false;
    }
    ...
    if (rc != SQLITE_DONE) {
        if (err) *err = str_printf("Update failed: %s", sqlite3_errmsg(data->db));
        return false;
    }
...
static bool sqlite_delete_row(DbConnection *conn, const char *table,
    ...
    if (rc != SQLITE_DONE) {
        if (err) *err = str_printf("Delete failed: %s", sqlite3_errmsg(data->db));
        return false;
    }
