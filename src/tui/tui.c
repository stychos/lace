...
bool tui_init(TuiState *state) {
  ...
  if (!state->header_win || !state->tab_win || !state->main_win ||
      !state->status_win) {
    tui_cleanup(state);
    return false;
  }
  ...
  state->content_rows =
      state->term_rows - 4; /* Header + table header + status + border */
...
static void tui_recreate_windows(TuiState *state) {
  ...
  int main_height =
      state->term_rows - 3; /* Total - header - tab bar - status */
...
/* Trim loaded data to keep memory bounded - keeps data within
 * TRIM_DISTANCE_PAGES of cursor */
static void tui_trim_loaded_data(TuiState *state) {
...
void tui_draw_status(TuiState *state) {
  ...
  if (state->sidebar_focused && state->tables && state->num_tables > 0) {
    ...
    size_t actual_idx =
        get_filtered_table_index(state, state->sidebar_highlight);
    ...
    if (col->default_val) {
      pos += snprintf(info + pos, sizeof(info) - pos, " DEFAULT %s",
                      col->default_val);
    }
...
void tui_draw_sidebar(TuiState *state) {
  ...
  for (size_t i = 0; i < state->num_tables && y < state->term_rows - 3; i++) {
    ...
    if (state->sidebar_filter_len > 0 &&
        !str_istr(name, state->sidebar_filter)) {
      continue;
    }
...
/* Get the filtered index from actual table index (returns 0 if not in filtered
 * list) */
static size_t get_sidebar_highlight_for_table(TuiState *state,
                                              size_t table_idx) {
  if (state->sidebar_filter_len == 0)
    return table_idx;
...
static size_t find_pk_columns(TuiState *state, size_t *pk_indices,
                              size_t max_pks) {
...
/* Set cell value directly (without edit mode) - set_null true for NULL, false
 * for empty string */
  ...
  while (state->running) {
    ...
    case 'h':
      ...
      if (state->cursor_col == 0 && state->sidebar_visible) {
        state->sidebar_focused = true;
        state->sidebar_highlight =
            get_sidebar_highlight_for_table(state, state->current_table);
      } else {
