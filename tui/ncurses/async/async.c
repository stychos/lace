...
static void *async_worker_thread(void *arg) {
  ...
  case ASYNC_OP_CONNECT:
    op->result = db_connect(op->client, op->connstr, &err);
    break;
  ...
  case ASYNC_OP_COUNT_ROWS: {
    ...
    if (db_count_rows(op->conn, op->table_name, NULL, 0, &count, &approximate, &err)) {
      op->count = (int64_t)count;
      op->is_approximate = approximate;
    } else {
      op->count = -1;
    }
    break;
  ...
  case ASYNC_OP_COUNT_ROWS_WHERE: {
    size_t count = 0;
    if (db_count_rows_where(op->conn, op->table_name, op->where_clause, &count, &err)) {
      op->count = (int64_t)count;
    } else {
      op->count = -1;
    }
...
void async_cancel(AsyncOperation *op) {
  ...
  /* If the operation is running on a connection, send cancel to daemon */
  if (op->state == ASYNC_STATE_RUNNING && op->conn) {
    /* Release mutex during network call to avoid blocking */
    lace_mutex_unlock(&op->mutex);

    /* Send cancel request to daemon - this will interrupt the running query */
    char *err = NULL;
    db_cancel_query(op->conn, &err);
    free(err);
