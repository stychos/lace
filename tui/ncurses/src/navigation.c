...
void tui_move_cursor(TuiState *state, int row_delta, int col_delta) {
  ...
  size_t loaded_rows = table_vm_row_count(vm);
  ...
  size_t loaded_count = table_vm_loaded_count(vm);
...
void tui_home(TuiState *state) {
  ...
  /* Update via viewmodel */
  table_vm_set_cursor(vm, 0, 0);
  table_vm_set_scroll(vm, 0, 0);
}
...
void tui_end(TuiState *state) {
  ...
  /* Get current column */
  size_t cursor_col;
  table_vm_get_cursor(vm, NULL, &cursor_col);

  /* Update via viewmodel */
  table_vm_set_cursor(vm, cursor_row, cursor_col);
  table_vm_set_scroll(vm, scroll_row, 0);
}
...
void tui_column_first(TuiState *state) {
  ...
  table_vm_get_cursor(vm, &cursor_row, NULL);
  table_vm_get_scroll(vm, &scroll_row, NULL);

  /* Update via viewmodel */
  table_vm_set_cursor(vm, cursor_row, 0);
  table_vm_set_scroll(vm, scroll_row, 0);
}
