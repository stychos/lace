 ...
 *
 * Query editor state is stored directly in Tab.
 *
...
void tui_draw_query(TuiState *state) {
  ...
  size_t query_cursor = tab->query_cursor;
  size_t query_scroll_line = tab->query_scroll_line;
  bool focus_results = ui->query_focus_results;
...
bool tui_handle_query_input(TuiState *state, const UiEvent *event) {
  ...
  if (hotkey_matches(cfg, event, HOTKEY_CUT_LINE)) {
    ...
    if (count > 0) {
      ...
      /* Copy to OS clipboard (redirect stderr to avoid error messages) */
